...
def get_raw_position(_source_positions, _sensor_positions):
    """
    Inputs: _source_positions [3 s 1] _sensor_positions [3 1 m]
    Output: [2 s]
    """
    x1 = _source_positions[0,:,0] - (_sensor_positions[0,:,1] + _sensor_positions[0,:,2]) / 2
    y1 = _source_positions[1,:,0] - (_sensor_positions[1,:,1] + _sensor_positions[1,:,2]) / 2

    theta0 = np.arctan2(_sensor_positions[1,:,1] - _sensor_positions[1,:,0], _sensor_positions[0,:,1] - _sensor_positions[0,:,0])
    x2 = x1 * np.cos(theta0) + y1 * np.sin(theta0) # [s,]
    y2 = y1 * np.cos(theta0) - x1 * np.sin(theta0) # [s,]
    
    doa = np.vstack((x2,y2)) # [2 s]
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self,idx):
        ...
        # _sensor_positions [3 m]  _source_positions [3 s]
        _source_positions = _source_positions[:,:,None]
        _sensor_positions = _sensor_positions[:,None,self.channel]
        # [2 s]
        doa = get_raw_position(_source_positions, _sensor_positions)
        doa = torch.from_numpy(doa.astype(np.float32))
        
        # mixture [c n] sources [s c n] doa [2 s]
        return mixture, sources, single_speaker, doa
        #return mixture, sources, single_speaker, mix_name, doa
