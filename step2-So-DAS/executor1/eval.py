...
parser.add_argument("--test_dir_css", type=str, required=True)

parser.add_argument("--save_wav_simu", type=int, default=0)
parser.add_argument("--save_wav_css", type=int, default=0)

parser.add_argument("--save_dir_simu", type=str, required=True)
parser.add_argument("--save_dir_css", type=str, required=True)

parser.add_argument("--use_gpu", type=int, default=0)
parser.add_argument("--do_mvdr", type=int, default=0)
...
def main1(model, conf):
    ...
    save_dir_simu_base = conf['save_dir_simu']
    ...
    for subset in ['0degree-20degree', '21degree-45degree', '46degree-90degree', '91degree-180degree']:
        ...
        save_dir_simu = os.path.join(save_dir_simu_base, subset)

        if not os.path.exists(save_dir_simu):
            os.makedirs(save_dir_simu)
        ...
        for idx in pbar:
            ...
            name, single_speaker, doa = tensors_to_device([str(test_wav['n']), test_wav['single_speaker'], torch.from_numpy(test_wav['doa'])], device=model_device)

            #mix = mix.permute(1,0) # [m n]
            if sources.dim() == 3:
                sources = sources[:,0]
                
            #ch_list = [0,1,2,3,4,5,6]
            #random.shuffle(ch_list)
            #print(ch_list)
            #mix = mix[ch_list]
            #ch0_id = ch_list.index(0)
            #print(ch0_id)
            # 2022.06.27
            ...
            est_doa, est_sources_7ch = model(mix[None], mix[None]) # [b s m n]
            ...
            if idx<10000:
                ...
                sf.write(os.path.join(save_dir_simu, name[:-4]+'_0.wav'), est_s1.squeeze() / est_s1.max(), conf["sample_rate"])
                est_s2 = est_sources_np[1]
                est_s2 *= np.max(np.abs(mix_np.squeeze()))/np.max(np.abs(est_s2))
                sf.write(os.path.join(save_dir_simu, name[:-4]+'_1.wav'), est_s2.squeeze() / est_s2.max(), conf["sample_rate"])
        ...
        all_metrics_df.to_csv(os.path.join(conf["exp_dir"], "all_metrics_permute_" + subset + ".csv"))
...
def main2(model, conf):
    ...
    for idx in tqdm(range(len(test_set))):
        ...
        est_sources, est_sources_7ch = model(mix[None], mix[None], do_eval=1)
        ...
        sf.write(os.path.join(save_dir_css, name+'_0.wav'), est_wav_s1.squeeze() / est_wav_s1.max(), conf["sample_rate"])
        sf.write(os.path.join(save_dir_css, name+'_1.wav'), est_wav_s2.squeeze() / est_wav_s2.max(), conf["sample_rate"])
