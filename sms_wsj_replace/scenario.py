...
def generate_sensor_positions(
    ...
    elif shape == 'circular_center':
        radius = scale
        delta_phi = 2 * np.pi / (number_of_sensors - 1)
        phi_0 = delta_phi / 2
        phi = np.arange(0, number_of_sensors-1) * delta_phi - phi_0
        sensor_positions_cir = np.asarray([
            radius * np.cos(phi),
            radius * np.sin(phi),
            np.zeros(phi.shape)
        ])
        sensor_positions_cen = np.asarray([
            [0],
            [0],
            [0]
        ])
        sensor_positions = np.hstack([sensor_positions_cen, sensor_positions_cir])
...
def generate_random_source_positions(
    ...
    x /= np.linalg.norm(x, axis=0)  # 单位方向向量
