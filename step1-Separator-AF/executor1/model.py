...
class MISO2(nn.Module):
    def __init__(self,
        ...
        self.extractor = FeatureExtractor(frame_len=self.frame_len, frame_hop=self.frame_hop, do_ipd=True, do_doa=True)
        self.bn = nn.BatchNorm1d(self.num_bins * self.fdim)

        self.in_Conv = nn.Sequential(
                nn.Conv2d(in_channels=2*self.fdim, out_channels=self.width, kernel_size=(1, 1)),
    ...
    def forward(self, mixture, doa): # mixture: [b m n] doa: [b 2 s]

        real_spec_src, imag_spec_src = self.extractor.stft(mixture, cplx=True) # [b m f t]

        #TODO
        #doa1, doa2 = torch.atan2(doa[:,1,0], doa[:,0,0]), torch.atan2(doa[:,1,1], doa[:,0,1])
        #doa = [doa1, doa2]
        doa = torch.atan2(doa[:,1,0], doa[:,0,0])

        _, _, feat = self.extractor(mixture, doa=doa) # [b (1lps+6sin+6cos+1doa=14)f t]
        B, nF, T = feat.size()
        feat = self.bn(feat).contiguous().view(B, nF//self.num_bins, self.num_bins, T).permute(0,1,3,2) # [b 14 t f]

        x = self.half_reshape(feat, False) # [b 2fdim t f=129]
...
class com_sisdr_loss1(nn.Module):
    ...
    def forward(self, est_targets, pha_mix, targets, SP): # est_targets [b s m n] pha_mix [b m f t] targets [b s m n] SP [b]
        ...
        est_targets, targets_recover = est_targets.permute(0,2,1,3).contiguous().view(B*M, S, N), targets_recover.permute(0,2,1,3).contiguous().view(B*M, S, N)
        SP = SP.repeat(M,1).transpose(0,1).contiguous().view(B*M)

        if (sum(SP)==0):
            sisdr_loss = self.sig_loss(est_targets, targets_recover)
        elif (sum(SP)==SP.shape[0]):
            sisdr_loss = 0.05 * self.sig_loss(est_targets[:,[0]], targets_recover[:,[0]])
        else:
            sisdr_loss = 0.05 * self.sig_loss(est_targets[SP == 1][:,[0]], targets_recover[SP == 1][:,[0]]) + self.sig_loss(est_targets[SP == 0], targets_recover[SP == 0])
